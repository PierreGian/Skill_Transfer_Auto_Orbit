...
inds = [0,0,0] #slow/medium/fast
for i,S in enumerate(IPIs_all):
    if(conds[i][0]=='fast' and conds[i][1]=='fast'): #fast condition
        FastNames[inds[2]] = names[i]
        inds[2] = inds[2] + 1
    elif(conds[i][0]=='medium' and conds[i][1]=='medium'): #medium condition
        MediumNames[inds[1]] = names[i]
        inds[1] = inds[1] + 1
    elif(conds[i][0]=='slow' and conds[i][1]=='slow'): #slow condition
        SlowNames[inds[0]] = names[i]
        inds[0] = inds[0] + 1
    for j,cyc in enumerate(S):
        for k,el in enumerate(cyc):
            if(Conditions_all[i][j][k]=='slow'):
                SlowIFI[i][GameNbs[i][j][k]].append(el)
            elif(Conditions_all[i][j][k]=='medium'):
                MediumIFI[i][GameNbs[i][j][k]].append(el)
            elif(Conditions_all[i][j][k]=='fast'):
...
for subj,ctup in enumerate(conds): 
    if(ctup[0]=='fast' and ctup[1]=='fast'): #fast condition
    ...
    elif(ctup[0]=='medium' and ctup[1]=='medium'): #medium condition
        for gg in range(GameNb):
    ...
    elif(ctup[0]=='slow' and ctup[1]=='slow'): #slow condition
        for gg in range(GameNb):
...
#1) Mu ISI
path = HumanPath
SeqFile = path+'MeanIFI_ALL_Humans.txt'
...
#Printing Medium
for subj in range(NbNames):
    cName = MediumNames[subj]+'\tmedium\tmedium'
    ToWrite = cName
    for gg in range(GameNb):
...
#Printing Slow
for subj in range(NbNames):
    cName = SlowNames[subj]+'\tslow\tslow'
    ToWrite = cName
    for gg in range(GameNb):
...
#2) std ISI
SeqFile = path+'StdIFI_ALL_Humans.txt'
ff_toWrite = open(SeqFile,"w+")
ToWrite = "Subject\tcondition1\tcondition2\t1\t2\t3\t4\t5\t6\t7\t8\t9\t10\t11\t12\t13\t14\t15\n"
ff_toWrite.write(ToWrite)
#Printing Fast
...
#3) CV ISI
SeqFile = path+'CVIFI_ALL_Humans.txt'
ff_toWrite = open(SeqFile,"w+")
ToWrite = "Subject\tcondition1\tcondition2\t1\t2\t3\t4\t5\t6\t7\t8\t9\t10\t11\t12\t13\t14\t15\n"
ff_toWrite.write(ToWrite)
#Printing Fast
